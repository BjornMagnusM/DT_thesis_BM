...
class AgentNodeTD3(DTROS):
    def __init__(self, node_name):
        ...
        #Added to detect obs shape 
        state_dict = checkpoint['actor_state_dict']
        self.first_layer = state_dict['encoder.convnet.0.weight']
        rospy.loginfo(f"shape of observations {self.first_layer.shape}")

        #Initilize the agent 
        self.actor = Actor(obs_shape=(self.first_layer.shape[1], 84, 84), action_dim=2).to(self.device)
